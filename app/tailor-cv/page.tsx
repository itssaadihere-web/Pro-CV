...
function TailorCvContent() {
  ...
  useEffect(() => {
    async function initPage() {
      ...
      try {
        ...
        // Case A: User is viewing a previously saved tailoring activity report
        if (activityId) {
        ...
        // Case B: User wants to tailor a new job opening -> Perform Pre-mount Credit Check (5 Credits required)
        ...
        // Fetch user's existing CV jobs for the dropdown select
  ...
  // Reset to form to tailor a new job opening
  ...
  // Submit action: Tailor CV Content (Deducts 5 Credits on CTA press)
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {/* VIEWING SAVED REPORT MODE */}
      {tailorResult && savedActivityDate ? (
        <div className="space-y-6">
          {/* Action Bar for Saved Activity */}
        ...
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
          ...
          {/* Primary Action Button (CTA Charges 5 Credits on click) */}
          <button
            type="button"
