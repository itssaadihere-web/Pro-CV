...
export default function HowItWorksClient() {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <main>
        {/* Hero Banner */}
        <section className="relative overflow-hidden bg-slate-950 text-white py-20 px-4 border-b border-slate-800">
          <div className="absolute inset-0 opacity-20" style={{
            backgroundImage: 'radial-gradient(circle at 30% 50%, #0f2b48 0%, transparent 60%), radial-gradient(circle at 70% 20%, #c5a059 0%, transparent 50%)'
          }} />
          <div className="relative max-w-3xl mx-auto text-center space-y-5">
            <motion.div
              initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              className="inline-flex items-center gap-2 rounded-full bg-slate-900 border border-[#c5a059]/30 px-4 py-1.5 text-xs font-black text-[#c5a059]"
            >
            ...
            <motion.h1
              ...
              className="text-4xl sm:text-5xl font-black tracking-tight leading-tight text-white"
            >
              From Upload to<br />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1.5">
                        <span className={`text-[11px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full ${
                          isDark ? 'bg-primary-900/80 text-[#c5a059] border border-primary-700/60' : 'bg-primary-50 text-primary border border-primary-200'
                        }`}>
                          Step {step.number}
                        </span>
                      </div>

                      <h2 className={`text-xl sm:text-2xl font-black leading-tight ${isDark ? 'text-white' : 'text-slate-950'}`}>
                        {step.title}
                      </h2>

                      <p className={`text-sm font-bold mt-1 mb-3 ${isDark ? 'text-[#c5a059]' : 'text-primary'}`}>
                        {step.subtitle}
                      </p>
                      ...
                      <ul className="space-y-2">
                        {step.checks.map((c, ci) => (
                          <li key={ci} className={`flex items-center gap-2.5 text-sm font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
                            <CheckCircle2 className={`w-4 h-4 shrink-0 ${isDark ? 'text-[#c5a059]' : 'text-primary'}`} />
                            {c}
                          </li>
                        ))}
                      </ul>
        ...
        <section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-slate-900">How Does Sophi AI Rewrite Resumes for ATS Compliance?</h2>
            </div>
            <div className="p-4 bg-primary-50/60 rounded-xl border border-primary-100 text-slate-800 text-sm leading-relaxed">
              <strong>Direct Answer:</strong> Sophi AI extracts text from your uploaded document or entry form, identifies missing job description keywords, converts bullet points into STAR-metric accomplishment statements (Action Verb + Task Context + Measurable Result), and exports the output into recruiter-approved single-column ATS layouts in under 60 seconds.
            </div>
          </div>
        </section>
        ...
        <section className="py-16 px-4 text-center">
          <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }}>
            ...
            <p className="text-slate-500 text-sm mb-7 max-w-sm mx-auto">
              Join thousands of professionals who've already landed their next role with <span className="font-black text-primary">SOPHI</span>.
            </p>
            <Link
              href="/login"
